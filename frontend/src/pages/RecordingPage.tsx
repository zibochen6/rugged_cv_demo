...
export default function RecordingPage({ language, onExit }: { language: Language; onExit: () => Promise<void> }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="card"><div className="mb-4 flex items-center gap-2"><Video aria-hidden="true" size={18} /><h2 className="font-semibold">{copy.recordingLibrary}</h2></div>
      {files.length === 0 ? <p className="text-sm text-muted">{copy.noRecordings}</p> : <div className="overflow-x-auto"><table className="recording-table"><thead><tr><th>{copy.cameraSource}</th><th>{copy.recordingStarted}</th><th>{copy.recordingElapsed}</th><th>{copy.recordingSize}</th><th aria-label={copy.actions} /></tr></thead><tbody>{files.map((file) => <tr key={file.id}><td>{cameraName(file.camera, language)}</td><td>{new Date(file.started_at * 1000).toLocaleString()}</td><td>{formatDuration(file.duration_s)}</td><td>{formatBytes(file.size_bytes)}</td><td><div className="flex justify-end gap-2"><button type="button" className="icon-button" onClick={() => setPlaying(file)} title={copy.playRecording} aria-label={copy.playRecording}><Play aria-hidden="true" size={18} fill="currentColor" /></button><a className="icon-button" href={getRecordingDownloadUrl(file.id)} title={copy.downloadRecording} aria-label={copy.downloadRecording}><Download aria-hidden="true" size={18} /></a></div></td></tr>)}</tbody></table></div>}
    </section>
