...
class EfficientTAMCameraPredictor(EfficientTAMBase):
    ...
    def _obj_id_to_idx(self, obj_id):
        ...
        # print("Current obj_ids: ", self.condition_state["obj_id_to_idx"])
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video", ncols=100):
    ...
    def clear_all_prompts_in_frame(
        self, frame_idx, obj_id, need_output=True
    ):
        ...
        if out is not None:
            ...
            self.condition_state["frames_tracked_per_obj"][obj_idx].pop(frame_idx, None)
    ...
    def remove_object(self, obj_id, strict=False, need_output=True):
        ...
        for frame_idx in obj_input_frames_inds:
            self.clear_all_prompts_in_frame(
                frame_idx, obj_id, need_output=False
            )
        ...
        _map_keys(self.condition_state["frames_tracked_per_obj"])
    ...
    def track(
        ...
        output_dict = self.condition_state["output_dict_per_obj"][obj_id]
    ...
    def track_multi_target(
        ...
        multi_target_output_dict = {}
        for idx in range(batch_size):
            output_dict = self.condition_state["output_dict_per_obj"][idx]
            # print("output_dict", output_dict)
            # obj_ids = self.condition_state["obj_ids"]
            obj_ids = idx
            current_out = self.track_step(
                frame_idx=self.frame_idx,
                is_init_cond_frame=False,
                current_vision_feats=[current_vision_feats[0][:, idx, :].unsqueeze(1)],
                current_vision_pos_embeds=[current_vision_pos_embeds[0][:, idx, :].unsqueeze(1)],
                feat_sizes=feat_sizes,
                point_inputs=None,
                mask_inputs=None,
                output_dict=output_dict,
            ...
            # output_dict[storage_key][self.frame_idx] = current_out
            self._manage_memory_obj(self.frame_idx, current_out, obj_id=idx)

            _, video_res_masks = self._get_orig_video_res_output(pred_masks_gpu)
            multi_target_output_dict[idx] = {
                "obj_id": obj_ids,
                "pred_masks": video_res_masks
            }
        return multi_target_output_dict


    def _manage_memory_obj(self, frame_idx, current_out, obj_id=0):
        output_dict = self.condition_state["output_dict_per_obj"][obj_id]
